...
public class MyDatabaseHelper extends SQLiteOpenHelper {
    ...
    void addBook(String title, int price, int stock, String code){
    ...
    void updateData(String row_id, String title, int price, int stock, String code){
        ...
        if(result == -1) Toast.makeText(context, "Failed to update", Toast.LENGTH_LONG).show();
        else Toast.makeText(context, "Update success", Toast.LENGTH_LONG).show();
    }

    void deleteOneRow(String row_id){
        SQLiteDatabase db = this.getWritableDatabase();
        long result = db.delete(TABLE_NAME, "_id=?",  new String[]{row_id});
        if(result == -1) Toast.makeText(context, "Failed to delete", Toast.LENGTH_LONG).show();
        else Toast.makeText(context, "Delete success", Toast.LENGTH_LONG).show();
    }
}
